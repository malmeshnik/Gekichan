...
from bot.utils.keyboards import (
    ...
    get_deadline_date_keyboard, get_deadline_time_keyboard,
...
async def view_task_internal(message: types.Message, task_id: str, api_client: APIClient, i18n: I18nContext):
    ...
    if 'project' in task and task['project']:
        ...
        task['project_name'] = project['name']
    text = render_task_detail(task, i18n)
    await message.answer(text, reply_markup=get_task_detail_keyboard(task_id, task['project'], i18n), parse_mode="HTML")
...
async def process_task_title(message: types.Message, state: FSMContext, i18n: I18nContext):
    ...
    builder = InlineKeyboardBuilder()
    builder.row(types.InlineKeyboardButton(text=i18n.get("common-skip"), callback_data="skip_description"))
    await message.answer(i18n.get("tasks-enter-description"), reply_markup=builder.as_markup())
    await state.set_state(TaskStates.waiting_for_description)

@router.message(TaskStates.waiting_for_description)
async def process_task_description(message: types.Message, state: FSMContext, i18n: I18nContext):
...
async def skip_task_description(callback: types.CallbackQuery, state: FSMContext, i18n: I18nContext):
    await state.update_data(description=None)
    await callback.message.answer(i18n.get("tasks-enter-priority"), reply_markup=get_priority_keyboard(i18n))
    await state.set_state(TaskStates.waiting_for_priority)
    await callback.answer()

@router.callback_query(TaskStates.waiting_for_priority, F.data.startswith("priority_"))
async def process_task_priority(callback: types.CallbackQuery, state: FSMContext, i18n: I18nContext):
    priority = callback.data.split("_")[-1]
    await state.update_data(priority=priority)
    await callback.message.answer(i18n.get("tasks-enter-deadline-date"), reply_markup=get_deadline_date_keyboard(i18n))
...
async def process_task_deadline_date(callback: types.CallbackQuery, state: FSMContext, api_client: APIClient, i18n: I18nContext):
    ...
    if choice == "custom":
        ...
        return
    date = now.date()
    if choice == "tomorrow": date = (now + datetime.timedelta(days=1)).date()
    elif choice == "weekend":
        days_ahead = 5 - now.weekday()
        if days_ahead <= 0: days_ahead += 7
        date = (now + datetime.timedelta(days=days_ahead)).date()
    elif choice == "nextweek": date = (now + datetime.timedelta(days=7 - now.weekday())).date()

    await state.update_data(deadline_date=date.isoformat())
    await callback.message.answer(i18n.get("tasks-enter-deadline-time"), reply_markup=get_deadline_time_keyboard(i18n))
    await state.set_state(TaskStates.waiting_for_deadline_time)
    await callback.answer()

@router.message(TaskStates.waiting_for_deadline_date)
async def process_custom_date(message: types.Message, state: FSMContext, i18n: I18nContext):
    try:
        date = datetime.datetime.strptime(message.text, "%Y-%m-%d").date()
        await state.update_data(deadline_date=date.isoformat())
        await message.answer(i18n.get("tasks-enter-deadline-time"), reply_markup=get_deadline_time_keyboard(i18n))
        await state.set_state(TaskStates.waiting_for_deadline_time)
    except ValueError:
...
async def process_task_deadline_time(callback: types.CallbackQuery, state: FSMContext, api_client: APIClient, i18n: I18nContext):
    choice = callback.data.split("_")[-1]
    now = datetime.datetime.now(datetime.timezone.utc)
    if choice == "custom":
        ...
        return
    time_str = "12:00"
    if choice == "plus1": time_str = (now + datetime.timedelta(hours=1)).strftime("%H:%M")
    elif choice == "plus2": time_str = (now + datetime.timedelta(hours=2)).strftime("%H:%M")
    elif choice == "morning": time_str = "09:00"
    elif choice == "evening": time_str = "18:00"
...
async def process_custom_time(message: types.Message, state: FSMContext, api_client: APIClient, i18n: I18nContext):
    try:
...
async def process_task_assignee(callback: types.CallbackQuery, state: FSMContext, api_client: APIClient, i18n: I18nContext):
    choice = callback.data.split("_")[-1]
    if choice == "skip":
        await state.update_data(assignee_id=None, assignee_name=i18n.get("common-unassigned"))
    else:
    ...
    text = i18n.get("tasks-confirm-create",
        title=data['title'],
    ...
    await safe_edit_or_answer(callback, text, reply_markup=get_confirmation_keyboard("task_confirm_yes", "task_confirm_no", i18n))
    await state.set_state(TaskStates.waiting_for_confirmation)
    await callback.answer()

@router.callback_query(TaskStates.waiting_for_confirmation, F.data == "task_confirm_yes")
async def task_confirm_yes(callback: types.CallbackQuery, state: FSMContext, api_client: APIClient, i18n: I18nContext):
    data = await state.get_data()
    deadline = None
    if data.get('deadline_date'):
    ...
    try:
        task = await api_client.create_task(
            callback.from_user.id, title=data['title'], project_id=data['project_id'],
            description=data.get('description'), priority=data['priority'],
            deadline=deadline, assignee=data.get('assignee_id')
        )
        ...
        builder = InlineKeyboardBuilder()
        builder.row(types.InlineKeyboardButton(text=i18n.get("common-skip"), callback_data=f"task_view_{task['id']}"))
        await callback.message.answer(i18n.get("tasks-upload-instruction"), reply_markup=builder.as_markup())
    except Exception:
...
async def task_confirm_no(callback: types.CallbackQuery, state: FSMContext, i18n: I18nContext):
    await state.clear()
...
async def list_attachments(callback: types.CallbackQuery, api_client: APIClient, i18n: I18nContext):
    ...
    for a in attachments:
        ...
        builder.row(types.InlineKeyboardButton(text=f"❌ {a['file_name']}", callback_data=f"task_attach_del_{a['id']}_{task_id}"))
    text = "\n".join(lines)
...
async def delete_attachment(callback: types.CallbackQuery, api_client: APIClient, i18n: I18nContext):
    parts = callback.data.split("_")
    attach_id, task_id = parts[3], parts[4]
    try:
        await api_client._request("DELETE", f"/api/attachments/{attach_id}/", user_id=callback.from_user.id)
...
async def process_attachment(message: types.Message, state: FSMContext, api_client: APIClient, i18n: I18nContext):
    ...
    file_id, file_name, mime_type, file_size = None, "attachment", "application/octet-stream", 0
    if message.document:
        file_id, file_name, mime_type, file_size = message.document.file_id, message.document.file_name, message.document.mime_type, message.document.file_size
    elif message.photo:
        file_id, file_name, mime_type, file_size = message.photo[-1].file_id, f"photo_{message.photo[-1].file_id}.jpg", "image/jpeg", message.photo[-1].file_size
    if file_id:
...
async def start_focus_from_task(callback: types.CallbackQuery, api_client: APIClient, i18n: I18nContext):
    ...
    await start_timer_callback_logic(callback, api_client, i18n, task_id=task_id, duration=1500)
...
async def show_edit_options(callback: types.CallbackQuery, i18n: I18nContext):
    task_id = callback.data.split("_")[-1]
    builder = InlineKeyboardBuilder()
    builder.row(types.InlineKeyboardButton(text="Title", callback_data=f"task_edfield_title_{task_id}"))
    builder.row(types.InlineKeyboardButton(text="Description", callback_data=f"task_edfield_desc_{task_id}"))
    builder.row(types.InlineKeyboardButton(text="Priority", callback_data=f"task_edfield_prio_{task_id}"))
    builder.row(types.InlineKeyboardButton(text="Deadline", callback_data=f"task_edfield_dead_{task_id}"))
    builder.row(types.InlineKeyboardButton(text="Assignee", callback_data=f"task_edfield_ass_{task_id}"))
    builder.row(types.InlineKeyboardButton(text=i18n.get("common-back"), callback_data=f"task_view_{task_id}"))
    await safe_edit_or_answer(callback, i18n.get("tasks-edit-select-field"), reply_markup=builder.as_markup())

@router.callback_query(F.data.startswith("task_edfield_"))
async def start_edit_field(callback: types.CallbackQuery, state: FSMContext, api_client: APIClient, i18n: I18nContext):
    parts = callback.data.split("_")
    ...
    if field == "title":
        ...
        await callback.message.answer(i18n.get("tasks-select-new-priority"), reply_markup=get_priority_keyboard(i18n, prefix="editprio"))
        await state.set_state(TaskStates.editing_priority)
    elif field == "dead":
        await callback.message.answer(i18n.get("tasks-select-new-deadline-date"), reply_markup=get_deadline_date_keyboard(i18n))
        await state.set_state(TaskStates.editing_deadline_date)
    elif field == "ass":
        task = await api_client.get_task(callback.from_user.id, task_id)
        project = await api_client.get_project(callback.from_user.id, task['project'])
        await callback.message.answer(i18n.get("tasks-select-new-assignee"), reply_markup=get_assignee_keyboard(project['members'], i18n))
        await state.set_state(TaskStates.editing_assignee)
    await callback.answer()

@router.message(TaskStates.editing_title)
async def process_edit_title(message: types.Message, state: FSMContext, api_client: APIClient, i18n: I18nContext):
    data = await state.get_data()
    await api_client.update_task(message.from_user.id, data['edit_task_id'], title=message.text)
...
async def process_edit_description(message: types.Message, state: FSMContext, api_client: APIClient, i18n: I18nContext):
    data = await state.get_data()
    await api_client.update_task(message.from_user.id, data['edit_task_id'], description=message.text)
...
async def process_edit_priority(callback: types.CallbackQuery, state: FSMContext, api_client: APIClient, i18n: I18nContext):
    priority = callback.data.split("_")[-1]
    data = await state.get_data()
    await api_client.update_task(callback.from_user.id, data['edit_task_id'], priority=priority)
...
async def process_edit_deadline_date(callback: types.CallbackQuery, state: FSMContext, api_client: APIClient, i18n: I18nContext):
    choice = callback.data.split("_")[-1]
    now = datetime.datetime.now(datetime.timezone.utc)
    if choice == "skip":
        data = await state.get_data()
        await api_client.update_task(callback.from_user.id, data['edit_task_id'], deadline=None)
        await callback.message.answer(i18n.get("tasks-deadline-removed"))
        await state.clear()
        return await view_task_internal(callback.message, data['edit_task_id'], api_client, i18n)
    if choice == "custom":
        await callback.message.answer(i18n.get("tasks-enter-custom-date"))
        await callback.answer()
        return
...
async def process_edit_custom_date(message: types.Message, state: FSMContext, i18n: I18nContext):
    try:
        date = datetime.datetime.strptime(message.text, "%Y-%m-%d").date()
        await state.update_data(edit_deadline_date=date.isoformat())
        await message.answer(i18n.get("tasks-enter-deadline-time"), reply_markup=get_deadline_time_keyboard(i18n))
...
async def process_edit_deadline_time(callback: types.CallbackQuery, state: FSMContext, api_client: APIClient, i18n: I18nContext):
    choice = callback.data.split("_")[-1]
    now = datetime.datetime.now(datetime.timezone.utc)
    if choice == "custom":
        await callback.message.answer(i18n.get("tasks-enter-custom-time"))
        ...
        return
    time_str = "12:00"
    if choice == "morning": time_str = "09:00"
    elif choice == "evening": time_str = "18:00"
    elif choice == "plus1": time_str = (now + datetime.timedelta(hours=1)).strftime("%H:%M")
    elif choice == "plus2": time_str = (now + datetime.timedelta(hours=2)).strftime("%H:%M")
    data = await state.get_data()
    deadline = f"{data['edit_deadline_date']}T{time_str}:00Z"
    await api_client.update_task(callback.from_user.id, data['edit_task_id'], deadline=deadline)
...
async def process_edit_custom_time(message: types.Message, state: FSMContext, api_client: APIClient, i18n: I18nContext):
    try:
        time_obj = datetime.datetime.strptime(message.text, "%H:%M").time()
        data = await state.get_data()
        deadline = f"{data['edit_deadline_date']}T{time_obj.strftime('%H:%M')}:00Z"
        await api_client.update_task(message.from_user.id, data['edit_task_id'], deadline=deadline)
...
async def process_edit_assignee(callback: types.CallbackQuery, state: FSMContext, api_client: APIClient, i18n: I18nContext):
    choice = callback.data.split("_")[-1]
    assignee_id = None if choice == "skip" else choice
    data = await state.get_data()
    await api_client.update_task(callback.from_user.id, data['edit_task_id'], assignee=assignee_id)
